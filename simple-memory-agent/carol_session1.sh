...
response=$(curl -s -X POST http://127.0.0.1:9090/invocation \
  ...
  -d '{"user_id": "carol_user", "run_id": "carol-session-1", "query": "Hi, I'\''m Carol. I'\''m a data scientist."}' \
...
response=$(curl -s -X POST http://127.0.0.1:9090/invocation \
  -H "Content-Type: application/json" \
  -d '{"user_id": "carol_user", "run_id": "carol-session-1", "query": "What programming languages do I like?"}' \
  | python3 -c "import sys,json; print(json.load(sys.stdin)['response'])")
echo "Agent: $response" >> carol_output.txt
echo "" >> carol_output.txt
...
response=$(curl -s -X POST http://127.0.0.1:9090/invocation \
  -H "Content-Type: application/json" \
  -d '{"user_id": "carol_user", "run_id": "carol-session-1", "query": "Do you know what Alice prefers?"}' \
  | python3 -c "import sys,json; print(json.load(sys.stdin)['response'])")
echo "Agent: $response" >> carol_output.txt
echo "" >> carol_output.txt
